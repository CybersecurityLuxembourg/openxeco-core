...
class TestRunDatabaseCompliance(BaseCase):
    ...
    def test_ok_entity_without_data(self, token):
        ...
        self.assertEqual(data_controls[0].value, "Value 'creation_date' of <ENTITY:1> is empty")
        self.assertEqual(data_controls[1].category, 'DATABASE COMPLIANCE')
        self.assertEqual(data_controls[1].value, "Value 'website' of <ENTITY:1> is empty")
        self.assertEqual(data_controls[2].category, 'DATABASE COMPLIANCE')
        self.assertEqual(data_controls[2].value, "Value 'image' of <ENTITY:1> is empty")
        self.assertEqual(data_controls[3].category, 'DATABASE COMPLIANCE')
        self.assertEqual(data_controls[3].value, '<ENTITY:1> has no address registered')
        self.assertEqual(data_controls[4].category, 'DATABASE COMPLIANCE')
        self.assertEqual(data_controls[4].value, '<ENTITY:1> has no phone number registered as a contact')
        self.assertEqual(data_controls[5].category, 'DATABASE COMPLIANCE')
        self.assertEqual(data_controls[5].value, '<ENTITY:1> has no email address registered  as a contact')

    @BaseCase.login
    @BaseCase.grant_access("/cron/run_database_compliance")
    def test_ok_entity_with_all_contact_and_address_data(self, token):
        s = self.db.tables["Setting"]
        self.db.insert({"property": "HIGHLIGHT_ENTITIES_WITHOUT_CREATION_DATE", "value": "TRUE"}, s)
        self.db.insert({"property": "HIGHLIGHT_ENTITIES_WITHOUT_WEBSITE", "value": "TRUE"}, s)
        self.db.insert({"property": "HIGHLIGHT_ENTITIES_WITHOUT_IMAGE", "value": "TRUE"}, s)
        ...
        self.db.insert({"id": 1, "name": "Entity", "website": "", "image": 11,
                        "creation_date": "2020-01-01", "description": "desc"}, self.db.tables["Entity"])
        self.db.insert({"id": 21, "entity_id": 1, "type": "EMAIL ADDRESS", "representative": "ENTITY",
                        "value": "mail@example.com"}, self.db.tables["EntityContact"])
        self.db.insert({"id": 22, "entity_id": 1, "type": "PHONE NUMBER", "representative": "ENTITY",
                        "value": "045065561"}, self.db.tables["EntityContact"])
        self.db.insert({"id": 31, "entity_id": 1, "address_1": "", "city": "",
                        "country": "", "latitude": 1, "longitude": 1}, self.db.tables["EntityAddress"])
