...
class TestGetMyFormAnswers(BaseCase):
    ...
    def test_ok(self, token):
        ...
        question3 = Serializer.serialize(self.db.insert({"form_id": form2['id'], "position": 1, "status": "ACTIVE"}, self.db.tables["FormQuestion"]), self.db.tables["FormQuestion"])
        ...
        self.db.insert({"form_question_id": question2['id'], "user_id": 2, "value": "YES"}, self.db.tables["FormAnswer"])
        self.db.insert({"form_question_id": question3['id'], "user_id": 1, "value": "YES"}, self.db.tables["FormAnswer"])

        response = self.application.get('/private/get_my_form_answers?form_id=' + str(form['id']),
    ...
    def test_ko_with_inactive_form(self, token):
        ...
        response = self.application.get('/private/get_my_form_answers?form_id=1',
                                        headers=self.get_standard_header(token))
