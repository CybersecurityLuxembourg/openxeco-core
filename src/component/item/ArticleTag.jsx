...
export default class ArticleTag extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevState.selectedCompanies !== this.state.selectedCompanies 
            ...
            if (prevState.selectedCompanies.length > this.state.selectedCompanies.length) {
                let removedEl = prevState.selectedCompanies.filter(x => !this.state.selectedCompanies.includes(x));
                this.deleteCompanyTag(removedEl[0])
            } else {
                let removedEl = this.state.selectedCompanies.filter(x => !prevState.selectedCompanies.includes(x));
        ...
        if (prevState.selectedTaxonomyValues !== this.state.selectedTaxonomyValues 
            ...
            if (prevState.selectedTaxonomyValues.length > this.state.selectedTaxonomyValues.length) {
                let removedEl = prevState.selectedTaxonomyValues.filter(x => !this.state.selectedTaxonomyValues.includes(x));
                this.deleteTaxonomyTag(removedEl[0])
            } else {
                let removedEl = this.state.selectedTaxonomyValues.filter(x => !prevState.selectedTaxonomyValues.includes(x));
    ...
    refresh() {
        ...
        getRequest.call(this, "article/get_article_tags/" + this.props.id, data => {
            this.setState({
                selectedCompanies: data.company_tags.map(t => { return t.id }),
                selectedTaxonomyValues: data.taxonomy_tags.map(t => { return t.id })
            });
        }, response => {
            nm.warning(response.statusText);
        }, error => {
