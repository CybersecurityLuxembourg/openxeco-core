...
import "./DialogImportUserAddresses.css";
...
export default class DialogImportUserAddresses extends React.Component {
	constructor(props) {
		super(props);
		...
		const defaultState = {
			...
			acceptedCommunicationOnly: true,
	...
	componentDidUpdate(_, prevState) {
		if (prevState.selectedEntities !== this.state.selectedEntities
			...
			|| prevState.acceptedCommunicationOnly !== this.state.acceptedCommunicationOnly) {
			this.getAddresses();
		}
	}
	...
	getAddresses() {
		...
		const params = {
			include_users: true,
			consider_communication_acceptance: this.state.acceptedCommunicationOnly,
	...
	render() {
		return (
			<Popup
				...
				trigger={
					<button>
						<i className="fas fa-user"/> Import from users...
					</button>
				}
				onOpen={() => this.onOpen()}
				modal
			>
				{(close) => <div className="row">
					<div className={"col-md-9"}>
						<h2><i className="fas fa-user"/> Import from users</h2>
					</div>
					...
					<div className={"col-md-12 row-spaced"}>
						<FormLine
							label={"User with accepted communication only"}
							type={"checkbox"}
							value={this.state.acceptedCommunicationOnly}
							onChange={(v) => this.changeState("acceptedCommunicationOnly", v)}
						/>
