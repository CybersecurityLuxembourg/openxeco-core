...
class UpdateDatabaseCompliance(Resource):
    ...
    def post(self):
        ...
        self.db.truncate(self.db.tables["DataAnomaly"])

        # Treat admin users

        admin_users = self.db.get(self.db.tables["User"], {"is_admin": True})

        anomalies += self._check_user_group_compliance(admin_users)
    ...
    def _check_user_group_compliance(self, users):
        anomalies = []
    ...
    def _check_company_address_compliance(self, companies):
        ...
        anomalies += [f"Company <{c.id}> has at least one address without geolocation" for c in companies_without_loc]
        ...
        for col in article_columns:
            ...
            if "VARCHAR" in col_type or "ENUM" in col_type:
                ...
                if len(empty_valued_articles) > 0:
                    anomalies += [f"Value '{col}' of article <{c.id}> is empty" for c in empty_valued_articles]
